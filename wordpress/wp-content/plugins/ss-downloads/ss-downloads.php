...
//version
define("SSDOWNLOADS_VERSION", "1.4.3");
...
function ssd_shortcode_handler($atts, $content=null, $code="") {
	...
	if(($require == "email" && $_SESSION['ssd_email_validates']) || ($require == "user" && $current_user->ID))
	{				
		if($delivery == "link")
		{		
			//show download box
...
function ssd_ss_downloads()
{
	...
	<div class="wrap">				
		...
		<textarea style="width: 500px; height: 100px;"><?php echo implode(", ", $collected_emails); ?></textarea>		
        ...
        <h2>
        	File Downloads
            <small>
				(<a target="_blank" href="<?php echo get_bloginfo("url"); ?>/wp-content/plugins/ss-downloads/services/exportdownloads.php">Download Full Report</a>)
